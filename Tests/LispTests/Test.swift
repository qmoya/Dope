...
//	}
	...
	@Test func testNestedVectors() async throws {
		...
		#expect(result == Vector(forms: [
			...
			Lisp.Form.vector(Lisp.Vector(forms: [
				Lisp.Form.literal(Lisp.Literal.number(Lisp.Number.integer(2))),
				Lisp.Form.literal(Lisp.Literal.number(Lisp.Number.integer(3))),
			])),
		]))
	}
...
//	}
	...
	@Test func testName() async throws {
		...
		let result = try parser.parse(&s)
		#expect(result == Name(
			headAndRest: .init(head: "f", rest: "oo"),
			tail: .init(components: ["bar.baz", "qux"])
		))
	}
	...
	@Test func testSimpleSymbol() async throws {
		...
		let result = try parser.parse(&s)
		#expect(result == SimpleSymbol.name(.init(
			headAndRest: .init(head: "f", rest: "oo"),
			tail: nil
		)))
	}
	...
	@Test func testNamespacedSymbol() async throws {
		...
		let result = try parser.parse(&s)
		#expect(result == NamespacedSymbol(
			namespace: .init(headAndRest: .init(head: "c", rest: "lojure"), tail: nil),
			symbol: .name(.init(headAndRest: .init(head: "f", rest: "oo"), tail: nil))
		))
	}
	...
	@Test func testSymbolNamespacedSymbol() async throws {
		...
		let result = try parser.parse(&s)
		#expect(result == Symbol.namespaced(NamespacedSymbol(
			namespace: .init(headAndRest: .init(head: "c", rest: "lojure"), tail: nil),
			symbol: .name(.init(headAndRest: .init(head: "f", rest: "oo"), tail: nil))
		)))
	}
	...
	@Test func testSymbolSimpleSymbol() async throws {
		...
		let parser = SymbolParser()
		let result = try parser.parse(&s)
		#expect(result == Symbol.simple(SimpleSymbol.name(.init(
			headAndRest: .init(head: "f", rest: "oo"),
			tail: nil
		))))
	}
	...
	@Test func testSimpleKeyword() async throws {
		...
		let result = try parser.parse(&s)
		#expect(result == SimpleKeyword(symbol: .simple(SimpleSymbol.name(.init(
			headAndRest: .init(head: "f", rest: "oo"),
			tail: nil
		)))))
	}
	...
	@Test func testSimpleNamespacedKeyword() async throws {
		...
		let parser = SimpleKeywordParser()
		let result = try parser.parse(&s)
		#expect(result == SimpleKeyword(symbol: .namespaced(.init(
			namespace: .init(headAndRest: .init(head: "f", rest: "oo"), tail: nil),
			symbol: .name(.init(headAndRest: .init(head: "b", rest: "ar"), tail: nil))
		))))
	}
	...
	//            Lisp.Form.list(
	//                Lisp.List(forms: [
	////                    Lisp.Form.literal(Lisp.Literal.symbol(Lisp.Symbol.simple(Lisp.SimpleSymbol.name(Lisp.Name(headAndRest:
	/// Lisp.Name.HeadAndRest(head: "+", rest: ""), tail: nil))))),
	//                    Lisp.Form.literal(Lisp.Literal.number(Lisp.Number.integer(1))),
	////                    Lisp.Form.literal(Lisp.Literal.number(Lisp.Number.integer(2)))
	//                ])
	//            )
